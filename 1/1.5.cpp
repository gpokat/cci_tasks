...
if(str1.length() == str2.length()){ //O(n)
int edit_seed=0;
...
if(askii_key1-askii_key2 !=0) edit_seed++;

}

if(edit_seed>1) return false;
else return true;
} else{
...
for(int i=0; i<bigger_str.length(); ++i){ //O(n)+-
string stmp=bigger_str;
string::iterator it=stmp.begin()+i;
stmp.erase(it);

if(stmp==smaller_str) return true;
